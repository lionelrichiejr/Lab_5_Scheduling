...
{ 
       ...
       * 1 Traverse until all process gets completely executed.
         ...
         - Completion time of *current process = curr_time +1;*
         - Calculate waiting time for each completed process. *wt[i]= Completion time - arrival_time-burst_time*
         - Increment time lap by one.
 
     */

      int i, proc_counter = 0, curr_time = 0, comp_time = 0; 
      int rem_bt[n];

      for(i = 0; i < n; i++){ rem_bt[i] = plist[i].bt; }

      while(proc_counter < n){
        // Find process with minimum remaining time at every single time lap.
        int min_index = 0;
        for(i = 1; i < n; i++){
